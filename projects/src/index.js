...
function findAllPSiblings(where) {
  ...
  for (let i = 0; i < where.childNodes.length - 1; i++) {
    let nextTagName = where.childNodes[i].nextElementSibling.tagName.toUpperCase();
    if (nextTagName == "P") array.push(where.childNodes[i]);
  }
  return array;
}
...
function deleteTextNodesRecursive(where) {
  ...
  if (where.hasChildNodes()) {
    ...
    for (const node of textNodes) where.removeChild(node);
  }
}
...
function collectDOMStat(root) {
  ...
  result.classes = new Proxy(result.classes, newProxy);
  result.tags = new Proxy(result.tags, newProxy);

  function addClassesAndTags(node) {
    for (let i = 0; i < node.classList.length; i++)
      result.classes[node.classList[i]]++;
  ...
  function collect(root) {
    for (const node of root.childNodes) {
      ...
      if (node.nodeType === 1) {
        addClassesAndTags(node);
        collect(node);
      }
    }
  }
  collect(root);
  //addClassesAndTags(root);
  return result;
}
